...
def get_tensorboard_callback(
    ...
    data_set_log_dir = LOG_DIR / Path(data_set_name.value) / Path(model_name) / Path(str(fold_index)) \
...
def get_checkpoint_file(
    ...
    data_set_checkpoint_dir = \
        CHECKPOINT_DIR / Path(data_set_name.value) / Path(model_name) / Path(str(fold_index)) / Path(str(learning_rate))
